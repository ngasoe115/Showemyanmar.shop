...
export const StoreProvider = ({ children }) => {
  ...
  // ==========================================
  // SUPABASE REALTIME & DATABASE INTEGRATION (SAFE WRAPPED)
  // ==========================================
  useEffect(() => {
    ...
    return () => {
      if (productSub && supabase) supabase.removeChannel(productSub);
      if (orderSub && supabase) supabase.removeChannel(orderSub);
    };
  }, []);
  ...
  const login = async (email, password) => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (error) {
          // Fall back to local check if email/password matches local test accounts
          const foundLocal = users.find((u) => u.email.toLowerCase() === email.toLowerCase() && u.password === password);
          if (foundLocal) {
            setCurrentUser(foundLocal);
            trustCurrentDevice(email);
            return { success: true, requiresOtp: false, user: foundLocal };
          }
        ...
        console.warn('Supabase login error fallback:', err);
      }
    }
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (!error && data?.user) {
          ...
          const loggedUser = {
            id: data.user.id,
            name: profile?.full_name || pendingOtp.userData?.name || data.user.email,
            email: data.user.email,
  ...
  const cancelOtpSession = () => setPendingOtp(null);
